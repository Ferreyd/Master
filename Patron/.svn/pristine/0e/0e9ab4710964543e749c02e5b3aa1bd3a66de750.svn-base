package tp2marc.bebetes;

import tp2damien.decorateurs.DecorateurBebete;
import tp2marc.decorateurs.DecorateurBebetes;
import tp2marc.simu.Actionnable;
import tp2marc.simu.Simulateur;
import tp2marc.visu.Dessinable;
import tp2marc.visu.Positionnable;
import tp2marc.visu.VisualisateurAnime;
...
public class ChampDeBebetes extends VisualisateurAnime
{
	...
	public ArrayList<? extends Bebete> fabriqueBebetes(int nb) {
		...
		for (int i = 0; i < nb; i++) {
			...
			Bebete newB = new BebeteEmergente(this, x, y, direction, vitesse,
					new Color(r, g, b));
			newB = new DecorateurBebetes(newB);
			// Le fameux de la capture impossible du joker ?
			nouvBebetes.add(newB);
		}
		return nouvBebetes;
	}
