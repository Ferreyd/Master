...
import culdesac.BebeteAvecElephant;
...
import tp2.damien.correction.strategyadapter.Agoraphobe;
import tp2.damien.correction.strategyadapter.Peureuse;
import tp2.damien.correction.strategybebete.Emergent;
import tp2.damien.correction.visu.Champ;

public class FactoryAgoraphobe extends BebeteFactory{

	Champ champ;
	
	
	public FactoryAgoraphobe(Champ leChamp){
		champ = leChamp;
	}
	...
	public ArrayList<Bebete> getBebetes(Champ champ, int nb) {
		...
		for (int i = 0; i < nb; i++) {
			...
			if (i%10 == 0) 
				{
				b1.setStrategy(new Agoraphobe());
				b1.setChampDeVue(Math.PI);

				b1 = new BebeteAvecEcho(b1);
				}
			else{
				b1.setStrategy(new Emergent());
			}
			//if(i%10 == 0) b1 = new BebeteAvecEcho(b1);
			nouvBebetes.add(b1);
		}
