...
public class FactoryPeureuse extends BebeteFactory{

	Champ champ;
	
	
	public FactoryPeureuse(Champ leChamp){
		champ = leChamp;
	}
	...
	public ArrayList<Bebete> getBebetes(Champ champ, int nb) {
		...
		for (int i = 0; i < nb; i++) {
			...
			if (i%10 == 0) 
				{
				b1.setStrategy(new Peureuse());
				b1 = new BebeteAvecNom(b1, "peureuse"+generateur.nextFloat());
				}
			else{
