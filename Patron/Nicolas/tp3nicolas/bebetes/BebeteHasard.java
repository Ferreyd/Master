...
public class BebeteHasard extends Bebete
{
    ...
    public static final int nbTourChgt = 30; // nombre de tours entre chaque changement au hasard de direction et de
    // vitesse
    protected int nbTour; // nombre de tours de la b�b�tes depuis le pr�c�dent changement de direction et de vitesse

    public BebeteHasard(Champ c, int x, int y, double dC, double vC, Color col)
    {
        champ = c;
        this.x = x;
        this.y = y;
    ...
    public void calculeDeplacementAFaire()
    {
        ...
        { // c'est le moment de changer de direction et de vitesse
            prochaineVitesse = vitesseCourante + ((Math.random() * 2) - 1);
