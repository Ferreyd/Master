...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template match="zoo:dauphin|zoo:requin">
        <tr>
            <td>
                <a name="{generate-id(zoo:nom)}">
                    <xsl:value-of select="zoo:nom"/>
                </a>
            </td>
            <td>
            ...
            <td>
                <xsl:value-of select="@espèce"/>
                <br/>
                <i>
                    <xsl:value-of select="@nom-savant"/>
                </i>
            </td>
            <td><xsl:value-of select="zoo:taille"/>&#160;<xsl:value-of select="zoo:taille/@unité"/>
            </td>
            <td><xsl:value-of select="zoo:poids"/>&#160;<xsl:value-of select="zoo:poids/@unité"/>
            </td>
            <td>
                <xsl:value-of select="zoo:sexe"/>
            </td>
            <xsl:if test="@photo">
                <td>
                    <img src="img/{@photo}" alt="{zoo:nom}"/>
                </td>
            </xsl:if>
        </tr>
    </xsl:template>
